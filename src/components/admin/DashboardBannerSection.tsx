...
import { Plus, Trash2, Pencil, ArrowUp, ArrowDown, Loader2, ImageIcon, Eye, EyeOff } from "lucide-react";
import { uploadCompressedImage } from "@/lib/imageCompress";
import { DashboardBannerSlider, type DashboardBannerSlide } from "@/components/DashboardBannerSlider";
...
export function DashboardBannerSection() {
  ...
  const reorderMut = useMutation({
    mutationFn: async ({ id, sort_order }: { id: string; sort_order: number }) => {
      const { error } = await supabase.from("dashboard_banner_slides").update({ sort_order }).eq("id", id);
      if (error) throw error;
    },
  ...
  const toggleMut = useMutation({
    mutationFn: async ({ id, is_active }: { id: string; is_active: boolean }) => {
      const { error } = await supabase.from("dashboard_banner_slides").update({ is_active }).eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
      ...
      <div className="grid lg:grid-cols-[1fr_420px] gap-5">
        ...
        <div>
          ...
          <p className="text-[11px] text-muted-foreground mt-2 leading-snug">
            💡 อัตราส่วนแนะนำ <span className="font-medium">16:5 หรือ 16:4</span> — สไลด์จะเล่นอัตโนมัติทุก 6 วินาที
          </p>
        </div>
      </div>
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <Input value={subtitle} onChange={(e) => setSubtitle(e.target.value)} placeholder="เช่น โปรโมชั่นเดือนนี้" className="h-10" maxLength={80} />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs">หัวเรื่องใหญ่ (title)</Label>
            <Textarea value={title} onChange={(e) => setTitle(e.target.value)} placeholder="เช่น ฟีเจอร์ใหม่: ตัวช่วยส่งใบเสนอราคา" rows={2} maxLength={160} />
          </div>
          <div className="space-y-1.5">
          ...
          <div className="flex items-center justify-between rounded-lg border p-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">เฉพาะสไลด์ที่เปิดอยู่จะแสดงบน Dashboard</p>
            </div>
