...
import {
  Wallet, Users,
  ArrowRight, TrendingUp, Receipt, Target, Check, Pencil,
} from "lucide-react";
...
export function OverviewTab({ onGo }: OverviewTabProps) {
  ...
  const monthIncome = finance.incomes.filter((i) => i.month === ym).reduce((s, i) => s + (i.gross || 0), 0);
  const ymPrefix = `${ym}-`;
...
function FeatureCard({
  icon: Icon, title, desc, badge, onClick,
}: { icon: React.ComponentType<{ className?: string }>; title: string; desc: string; badge?: string; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
...
function MonthlyGoalCard({
  ...
  React.useEffect(() => { if (!editing) setDraft(String(goal || "")); }, [goal, editing]);
      ...
      <CardContent className="p-5 sm:p-6">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-3 min-w-0">
            <div className={`rounded-xl p-2.5 ${reached ? "bg-success/15 text-success" : "bg-primary-soft text-primary"}`}>
              <Target className="h-5 w-5" />
            </div>
            <div className="min-w-0">
              <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">เป้าหมายรายได้เดือนนี้</p>
              {!editing ? (
                <p className="text-xl sm:text-2xl font-semibold num tracking-tight mt-0.5">
                  ฿{fmt(income)}
                  {goal > 0 && <span className="text-muted-foreground text-sm font-normal"> / ฿{fmt(goal)}</span>}
                </p>
              ) : (
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            {!editing ? (
              ...
              <>
                ...
                <Button variant="ghost" size="sm" onClick={() => setEditing(false)} className="h-8">ยกเลิก</Button>
              </>
            )}
          </div>
        </div>
