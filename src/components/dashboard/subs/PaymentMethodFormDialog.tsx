...
export function PaymentMethodFormDialog({
  ...
  const [providerId, setProviderId] = React.useState<string>(
    initial ? PRESET_PAYMENT_PROVIDERS.find((p) => p.label === initial.label)?.id ?? "custom" : "kbank",
  );
  ...
  const [statementDay, setStatementDay] = React.useState<string>(initial?.statementDay ? String(initial.statementDay) : "");
        ...
        <div className="space-y-4">
          <div className="grid gap-2">
            ...
            <Select value={providerId} onValueChange={setProviderId}>
              ...
              <SelectContent className="max-h-[280px]">
                <div className="px-2 py-1 text-[10px] font-semibold uppercase text-muted-foreground">ธนาคาร</div>
                {PRESET_PAYMENT_PROVIDERS.filter((p) => p.type !== "wallet").map((p) => (
                  <SelectItem key={p.id} value={p.id}>{p.label}</SelectItem>
                ))}
                <div className="px-2 py-1 text-[10px] font-semibold uppercase text-muted-foreground mt-1">E-Wallet / BNPL</div>
                {PRESET_PAYMENT_PROVIDERS.filter((p) => p.type === "wallet").map((p) => (
                  <SelectItem key={p.id} value={p.id}>{p.label}</SelectItem>
                ))}
                <div className="px-2 py-1 text-[10px] font-semibold uppercase text-muted-foreground mt-1">อื่นๆ</div>
          ...
          {providerId === "custom" && (
            <div className="grid gap-2">
              ...
              <Input value={customLabel} maxLength={60} onChange={(e) => setCustomLabel(e.target.value)} placeholder="เช่น KBank Platinum" />
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            {type !== "cash" && (
              <div className="grid gap-2">
                ...
                <Input className="num" maxLength={4} value={last4} onChange={(e) => setLast4(e.target.value.replace(/\D/g, ""))} placeholder="1234" />
              </div>
            )}
          </div>
