...
export function CategoryDonut({ subs }: { subs: Subscription[] }) {
  const byCategory = React.useMemo(() => {
    ...
    return Object.entries(map)
      .map(([name, v]) => ({ name, value: v.total, items: v.items.sort((a, b) => b.amount - a.amount) }))
  ...
  return (
    <Card className="animate-fade-up">
      <CardHeader className="pb-2"><CardTitle className="text-base">สัดส่วนตามหมวด</CardTitle></CardHeader>
      <CardContent>
        <div className="h-[200px] sm:h-[220px]">
          <ResponsiveContainer>
            <PieChart>
              <Pie data={byCategory} dataKey="value" innerRadius="55%" outerRadius="85%" paddingAngle={3} stroke="none">
                {byCategory.map((c) => <Cell key={c.name} fill={CATEGORY_COLORS[c.name] || "var(--color-chart-1)"} />)}
              </Pie>
              <Tooltip
            ...
            return (
              ...
              >
                <CollapsibleTrigger className="w-full flex items-center gap-2 text-xs rounded-lg px-2 py-1.5 hover:bg-muted/50 transition-colors">
                  ...
                  <span className="text-[10px] text-muted-foreground/70 shrink-0">({c.items.length})</span>
                  <span className="ml-auto num font-medium shrink-0">฿{formatTHB(c.value)}</span>
                  <ChevronDown className={`h-3.5 w-3.5 text-muted-foreground transition-transform ${isOpen ? "rotate-180" : ""}`} />
                </CollapsibleTrigger>
                <CollapsibleContent>
                  <div className="pl-6 pr-2 py-1 space-y-1 border-l-2 ml-3 mt-1" style={{ borderColor: color }}>
                    {c.items.map((item) => (
                      <div key={item.id} className="flex items-center gap-2 text-[11px]">
                        <span className="truncate flex-1">{item.name}</span>
                        <span className="num text-muted-foreground shrink-0">฿{formatTHB(item.amount)}</span>
                      </div>
                    ))}
                  </div>
                </CollapsibleContent>
