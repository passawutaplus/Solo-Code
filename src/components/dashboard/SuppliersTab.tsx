...
import {
  Plus, Search, Trash2, Pencil, ExternalLink, FileText,
...
export function SuppliersTab() {
  const { suppliers, files, links, isLoading, create, update, remove,
    uploadFile, deleteFile, addLink, removeLink, getSignedUrl,
    enableShare, disableShare } = useSuppliers();
  ...
  return (
    <div className="space-y-4">
        ...
        <CardContent className="p-4 sm:p-5">
          <div className="flex items-start justify-between gap-3 flex-wrap mb-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                ร้านประจำ · สตูดิโอ · พาร์ทเนอร์ — เก็บคอนแทกต์, เรตราคา, ลิงก์ และแชร์เป็นนามบัตรได้
              </p>
            </div>
          ...
          <div className="flex items-center gap-2 mb-4 flex-wrap">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <Input value={query} onChange={(e) => setQuery(e.target.value)}
                placeholder="ค้นหาชื่อ / ผู้ติดต่อ / แท็ก" className="pl-9 h-9" />
            </div>
            <div className="flex flex-wrap gap-1">
              <Chip active={filterCat === "all"} onClick={() => setFilterCat("all")}>ทั้งหมด</Chip>
              {SUPPLIER_CATEGORIES.map((c) => {
                const Icon = c.icon;
                return (
                  <Chip key={c.key} active={filterCat === c.label} onClick={() => setFilterCat(c.label)}>
                    <Icon className="h-3 w-3 mr-1 inline-block" />{c.label}
                  </Chip>
                );
              })}
            </div>
          ...
          {!isLoading && filtered.length === 0 && (
            <div className="text-center py-12 rounded-xl border border-dashed border-border/60">
              ...
              <p className="text-sm text-muted-foreground mb-3">
                {suppliers.length === 0 ? "ยังไม่มี Supplier — เริ่มเพิ่มผู้ขายรายแรก" : "ไม่พบรายการที่ค้นหา"}
              </p>
              {suppliers.length === 0 && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
            {filtered.map((s) => {
              ...
              return (
                <SupplierCard
                  key={s.id} s={s} files={sFiles} links={sLinks}
                  onEdit={() => setEditing(s)}
      ...
      <SupplierFormDialog
        ...
        onSubmit={async (data, staged) => {
          if (editing) {
            ...
            for (const l of staged.links) {
              try { await addLink(created.id, l.label, l.url); } catch { /* ignore */ }
            }
            for (const f of staged.files) {
              try { await uploadFile(created.id, f); } catch { /* ignore */ }
            }
        ...
        onUpload={async (file) => { if (editing) { await uploadFile(editing.id, file); } }}
        onDeleteFile={async (f) => { await deleteFile(f as never); }}
        onAddLink={async (label, url) => { if (editing) { await addLink(editing.id, label, url); } }}
        onRemoveLink={async (id) => { await removeLink(id); }}
...
function SupplierCard({
  s, files, links, onEdit, onDelete, onShare, onPreview, onOpenFile,
}: {
  s: Supplier;
  files: { id: string; fileName: string; storagePath: string; mimeType?: string }[];
  links: { id: string; label: string; url: string }[];
  onEdit: () => void; onDelete: () => void; onShare: () => void; onPreview: () => void;
    ...
    <div className="rounded-xl border border-border/60 bg-card p-3.5 hover:shadow-elevated hover:-translate-y-0.5 transition-all flex flex-col gap-2.5 overflow-hidden">
      ...
      <div className="flex items-start justify-between gap-2">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {s.rating > 0 && (
              <span className="inline-flex items-center gap-0.5 text-[11px] text-amber-600">
                <Star className="h-3 w-3 fill-amber-500 stroke-amber-500" />{s.rating}
              </span>
            )}
            {s.isShared && (
        ...
        <div className="flex gap-0.5 shrink-0">
          <Button size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={onPreview} title="พรีวิว">
          ...
          <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-destructive" onClick={onDelete} title="ลบ">
            <Trash2 className="h-3.5 w-3.5" />
          </Button>
        </div>
      </div>

      {s.contactName && <p className="text-xs text-muted-foreground">ผู้ติดต่อ: {s.contactName}</p>}
      ...
      <div className="text-[11px] text-muted-foreground space-y-0.5">
        ...
        {s.website && safeHref(s.website) && (
          <div className="flex items-center gap-1.5 truncate">
            ...
            <a href={safeHref(s.website)!} target="_blank" rel="noopener" className="truncate hover:text-primary">
        ...
        {s.mapUrl && safeHref(s.mapUrl) && (
          <div className="flex items-center gap-1.5 truncate">
            ...
            <a href={safeHref(s.mapUrl)!} target="_blank" rel="noopener" className="truncate hover:text-primary">
              เปิด Google Maps
            </a>
          </div>
        )}
      ...
      {s.rateNote && (
        <div className="rounded-md bg-muted/40 px-2 py-1 text-[11px]">
          <span className="text-muted-foreground">เรต: </span>{s.rateNote}
        </div>
      )}
      ...
      {s.tags.length > 0 && (
        <div className="flex flex-wrap gap-1">
          {s.tags.map((t) => (
            <span key={t} className="text-[10px] rounded-full bg-primary-soft text-primary px-2 py-0.5">#{t}</span>
          ))}
        </div>
      )}
      ...
      {(files.length > 0 || links.length > 0) && (
        <div className="border-t border-border/40 pt-2 space-y-1">
          {links.map((l) => {
            ...
            if (!safe) return null;
            return (
              <a key={l.id} href={safe} target="_blank" rel="noopener"
          ...
          {files.map((f) => (
            <button key={f.id} onClick={() => onOpenFile(f.storagePath)}
              className="flex items-center gap-1.5 text-[11px] text-foreground hover:text-primary truncate w-full text-left">
              <FileText className="h-3 w-3 shrink-0" />{f.fileName}
            </button>
          ))}
        </div>
      )}

      {s.notes && <p className="text-[11px] text-muted-foreground line-clamp-2 border-t border-border/40 pt-2">{s.notes}</p>}
    </div>
  );
}
