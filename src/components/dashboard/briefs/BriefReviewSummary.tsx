    ...
    <div className="rounded-2xl border-2 border-dashed border-border bg-card p-5 sm:p-7 space-y-5 font-sans">
      ...
      <div className="text-center border-b border-dashed border-border pb-4 space-y-1">
        ...
        <p className="text-[11px] text-muted-foreground">
          จัดทำโดย {o.brand_name || o.display_name || "Freelancer"} · อัปเดต {new Date(brief.updated_at).toLocaleString("th-TH")}
        </p>
      </div>

      {/* Sections */}
      <Row label="ลูกค้า / แบรนด์" value={[ci.client_name, ci.brand_name].filter(Boolean).join(" · ")} />
      <Row label="ติดต่อ" value={[ci.contact_email, ci.contact_phone, ci.contact_line].filter(Boolean).join(" · ")} />
      <Divider />
      ...
      {(po.scope_items ?? []).length > 0 && (
        <div className="space-y-1.5">
          <div className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground">เนื้องาน & สโคป</div>
              ...
              <tbody>
                {(po.scope_items ?? []).map((it, i) => (
                  <tr key={i} className="border-b last:border-b-0">
                    ...
                    <td className="px-2.5 py-1.5 text-muted-foreground hidden sm:table-cell">{it.note ?? ""}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <Divider />
      <Row label="กลุ่มเป้าหมาย" value={[au.gender, au.age_range, au.lifestyle, au.interests].filter(Boolean).join(" · ")} />
      <Divider />
      <Row label="Mood & Tone" value={(dd.moods ?? []).join(", ")} />
      <SwatchRow label="🎨 สีที่ชอบ" colors={dd.liked_color_chips ?? []} fallback={dd.liked_colors} />
      <SwatchRow label="🚫 สีต้องห้าม" colors={dd.forbidden_color_chips ?? []} fallback={dd.forbidden_colors} />
      <Row label="ฟอนต์" value={dd.liked_fonts} />
      <Row label="แรงบันดาลใจ" value={dd.inspiration} />
      {brief.references.length > 0 && (
        <div className="space-y-1.5">
          <div className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground">รูปอ้างอิง ({brief.references.length})</div>
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
            {brief.references.map((r, i) => (
              <img key={i} src={r.url} alt="" className="aspect-square w-full object-cover rounded-lg border border-border" />
            ))}
          </div>
        </div>
      )}
          ...
          <div className="rounded-xl bg-emerald-500/5 border border-emerald-500/30 p-4 flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              {brief.confirmed_signature && (
                <div className="text-2xl italic font-[cursive] text-foreground/80 mt-1">{brief.confirmed_signature}</div>
              )}
              <div className="text-[11px] text-muted-foreground mt-1">
                ยืนยันเมื่อ {new Date(brief.confirmed_at).toLocaleString("th-TH", { dateStyle: "long", timeStyle: "short" })}
              </div>
            </div>
          </div>
        </div>
...
function Row({ label, value, multiline }: { label: string; value?: string | null; multiline?: boolean }) {
  ...
  const isEmpty = v === "—";
  return (
    <div className={`grid grid-cols-[110px_1fr] gap-3 text-sm ${multiline ? "items-start" : "items-baseline"}`}>
      <div className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground pt-0.5">{label}</div>
      <div className={`${isEmpty ? "text-muted-foreground/80 italic" : "text-foreground"} ${multiline ? "whitespace-pre-wrap" : "truncate"}`}>{v}</div>
    </div>
  );
}
...
function SwatchRow({ label, colors, fallback }: { label: string; colors: string[]; fallback?: string }) {
      ...
      <div className="space-y-1.5">
        {colors.length > 0 && (
          <div className="flex flex-wrap gap-1.5">
            {colors.map((c) => (
              <div key={c} className="flex items-center gap-1 rounded-md border border-border bg-card px-1.5 py-1">
