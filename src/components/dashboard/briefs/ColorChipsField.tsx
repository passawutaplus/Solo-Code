...
export function ColorChipsField({ value, onChange, disabled, placeholder, label }: Props) {
  ...
  const add = (hex: string) => {
    ...
    if (!n) { toast.error("Hex code ไม่ถูกต้อง (เช่น #FF6B00)"); return; }
    if (value.includes(n)) { toast.info("มีสีนี้อยู่แล้ว"); return; }
    onChange([...value, n]);
    setHexInput("");
  };
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap gap-1.5 items-center">
        {value.length === 0 && (
          <span className="text-xs text-muted-foreground">{placeholder ?? "ยังไม่มีสี กดปุ่ม + เพื่อเพิ่ม"}</span>
        )}
        {value.map((hex) => (
          <div
            key={hex}
          ...
          >
            ...
            <button type="button" onClick={() => copy(hex)} className="text-muted-foreground hover:text-foreground">
        ...
        {!disabled && (
          <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
              <Button type="button" variant="outline" size="sm" className="h-7 rounded-lg gap-1 text-xs">
            ...
            <PopoverContent className="w-64 p-3 space-y-3" align="start">
              <div
                ...
                style={{ backgroundColor: picked, color: preferWhiteText(picked) ? "#fff" : "#000" }}
              >
              ...
              <div className="space-y-1.5">
                ...
                <Button type="button" size="sm" onClick={() => { add(picked); setOpen(false); }} className="w-full">
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex gap-1.5">
                  <Input
                    ...
                    onKeyDown={(e) => { if (e.key === "Enter") { add(hexInput); setOpen(false); } }}
                  />
                  <Button type="button" size="sm" variant="outline" onClick={() => { add(hexInput); setOpen(false); }}>เพิ่ม</Button>
                </div>
              </div>
            </PopoverContent>
          </Popover>
