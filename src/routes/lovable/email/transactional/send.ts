  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        // SECURITY: When template does not define a fixed `to`, restrict the
        // caller-supplied recipient to the authenticated user's own email.
        // Prevents abuse where any logged-in user could send branded emails to
        // arbitrary third-party inboxes (phishing vector).
        if (!template.to) {
          const userEmail = user.email?.toLowerCase().trim();
          if (!userEmail || userEmail !== effectiveRecipient.toLowerCase().trim()) {
            console.warn("Refused cross-recipient email send", {
              templateName,
              user_redacted: redactEmail(user.email),
              requested_redacted: redactEmail(effectiveRecipient),
            });
            return Response.json(
        ...
        // SECURITY: Validate any URL fields in templateData against an allowlist.
        // Prevents attackers from injecting phishing links into trusted branded emails.
        ...
        for (const field of URL_FIELDS) {
          ...
          if (typeof v === "string" && v.length > 0) {
            const ok = ALLOWED_URL_ORIGINS.some((o) => v.startsWith(o + "/") || v === o);
            if (!ok) {
              console.warn("Stripped disallowed URL from templateData", {
                templateName,
                field,
              });
