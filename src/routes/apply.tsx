...
export const Route = createFileRoute("/apply")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "เปิดรับสมัคร Tester รุ่นแรก จำกัด 100 คน เพื่อทดลองใช้เครื่องมือผู้ช่วยฟรีแลนซ์ก่อนใคร" },
...
const schema = z.object({
  ...
  (d) => (d.contact_email && d.contact_email.length > 0) || (d.contact_line && d.contact_line.length > 0),
  { message: "กรุณาระบุอย่างน้อย 1 ช่องทางติดต่อ (Email หรือ LINE)" }
).refine(
  (d) => !d.contact_email || /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(d.contact_email),
  { message: "รูปแบบอีเมลไม่ถูกต้อง", path: ["contact_email"] }
);
...
function ApplyPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    const payload: Record<string, unknown> = {
      ...
      main_field_other: parsed.data.main_field === "อื่นๆ" ? parsed.data.main_field_other || null : null,
      ...
      quotation_method_other: parsed.data.quotation_method.includes("อื่นๆ") ? parsed.data.quotation_method_other || null : null,
      pain_points: parsed.data.pain_points,
      pain_points_other: parsed.data.pain_points.includes("อื่นๆ") ? parsed.data.pain_points_other || null : null,
      ...
      <header className="sticky top-0 z-30 glass border-b border-white/40">
        <div className="mx-auto max-w-3xl px-4 py-3 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-2.5">
            <img src={logoUrl} alt="" className="h-9 w-9 rounded-xl object-cover shadow-soft ring-1 ring-white/60"  loading="lazy" decoding="async" />
            <div>
      ...
      <main className="relative z-10 mx-auto max-w-3xl px-4 py-8 sm:py-12">
        ...
        <div className="text-center mb-8">
          ...
          <p className="mt-4 max-w-xl mx-auto text-sm sm:text-base text-muted-foreground leading-relaxed">
            So1o Freelancer เปิดรับสมัคร Tester รุ่นแรก (จำกัด 100 user) เพื่อทดลองใช้เครื่องมือผู้ช่วยฟรีแลนซ์
          ...
          <CardContent className="p-5 sm:p-7">
            <form onSubmit={onSubmit} className="space-y-7">
              ...
              <Section num="1" title="ข้อมูลพื้นฐาน" desc="Basic Info">
                <div className="grid sm:grid-cols-2 gap-4">
                  <Field label="ชื่อ-นามสกุล" required>
                    <Input value={form.full_name} onChange={(e) => setForm({ ...form, full_name: e.target.value })} placeholder="เช่น สมชาย ใจดี" />
                  </Field>
                  <Field label="ชื่อในวงการ (ถ้ามี)">
                    <Input value={form.alias_name} onChange={(e) => setForm({ ...form, alias_name: e.target.value })} placeholder="เช่น Chai.designs" />
                  </Field>
                </div>

                <Field label="สายงานหลักของคุณคืออะไร?" required>
                  <RadioGroup value={form.main_field} onValueChange={(v) => setForm({ ...form, main_field: v })} className="grid grid-cols-2 sm:grid-cols-5 gap-2">
                    {FIELDS.map((f) => (
                      <label key={f} className={`flex items-center gap-2 rounded-lg border px-3 py-2 cursor-pointer text-sm transition-colors ${form.main_field === f ? "border-primary bg-primary-soft" : "border-border hover:bg-muted/50"}`}>
                  ...
                  {form.main_field === "อื่นๆ" && (
                    <Input className="mt-2" placeholder="โปรดระบุสายงาน" value={form.main_field_other} onChange={(e) => setForm({ ...form, main_field_other: e.target.value })} />
                  )}
                </Field>

                <Field label="เป็นฟรีแลนซ์มาแล้วกี่ปี" required>
                  <RadioGroup value={form.years_experience} onValueChange={(v) => setForm({ ...form, years_experience: v })} className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    {YEARS.map((y) => (
                      <label key={y} className={`flex items-center gap-2 rounded-lg border px-3 py-2 cursor-pointer text-sm transition-colors ${form.years_experience === y ? "border-primary bg-primary-soft" : "border-border hover:bg-muted/50"}`}>
                ...
                <div className="space-y-1.5">
                  <Label className="text-sm font-medium">
                    ...
                    <span className="text-[11px] text-muted-foreground font-normal ml-2">(กรอกอย่างน้อย 1 ช่องทาง)</span>
                  </Label>
              ...
              <Section num="2" title="เจาะลึกพฤติกรรม" desc="User Behavior">
                <Field label="ปกติคุณออกใบเสนอราคาด้วยวิธีใด?" required help="เลือกได้มากกว่า 1 ข้อ">
                  <div className="grid sm:grid-cols-2 gap-2">
                    {QUOTE_METHODS.map((m) => (
                      <label key={m} className={`flex items-start gap-2.5 rounded-lg border px-3 py-2.5 cursor-pointer text-sm transition-colors ${form.quotation_method.includes(m) ? "border-primary bg-primary-soft" : "border-border hover:bg-muted/50"}`}>
                        <Checkbox checked={form.quotation_method.includes(m)} onCheckedChange={() => toggleArr("quotation_method", m)} />
                        <span>{m}</span>
                      </label>
                    ))}
                  </div>
                  {form.quotation_method.includes("อื่นๆ") && (
                    <Input className="mt-2" placeholder="โปรดระบุวิธีอื่น" value={form.quotation_method_other} onChange={(e) => setForm({ ...form, quotation_method_other: e.target.value })} />
                  )}
                </Field>

                <Field label="ความทรมาน (Pain Point) ที่สุดของคุณเวลาคุยงานคืออะไร?" help="เลือกกี่ข้อก็ได้">
                  <div className="grid sm:grid-cols-2 gap-2">
                    {PAINS.map((p) => (
                      <label key={p} className={`flex items-start gap-2.5 rounded-lg border px-3 py-2.5 cursor-pointer text-sm transition-colors ${form.pain_points.includes(p) ? "border-primary bg-primary-soft" : "border-border hover:bg-muted/50"}`}>
                        <Checkbox checked={form.pain_points.includes(p)} onCheckedChange={() => toggleArr("pain_points", p)} />
                        <span>{p}</span>
                      </label>
                    ))}
                  </div>
                  {form.pain_points.includes("อื่นๆ") && (
                    <Input className="mt-2" placeholder="โปรดระบุ pain point อื่น" value={form.pain_points_other} onChange={(e) => setForm({ ...form, pain_points_other: e.target.value })} />
                  )}
                </Field>
              </Section>
              ...
              <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-3 sm:p-4 text-xs sm:text-sm text-foreground/90 leading-relaxed">
                ...
                <p>
                  สิทธิ์การเข้าใช้งานมีจำนวนจำกัด ทีมงานขออนุญาตระงับสิทธิ์ผู้ที่ไม่มีความเคลื่อนไหวเกิน 7 วัน เพื่อส่งต่อโอกาสให้ผู้ที่อยู่ใน Waitlist ท่านถัดไปครับ
                </p>
              </div>
              ...
              <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-border pt-5">
                ...
                <Button type="submit" disabled={submitting} className="bg-gradient-primary text-primary-foreground rounded-full px-6 shadow-elevated">
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-end gap-3 border-b border-border pb-2">
        <span className="text-2xl font-bold bg-gradient-primary bg-clip-text text-transparent leading-none">{num}</span>
        <div>
  ...
  return (
    <div className="space-y-1.5">
      <Label className="text-sm font-medium">
        ...
        {help && <span className="text-[11px] text-muted-foreground font-normal ml-2">({help})</span>}
      </Label>
      {children}
    </div>
  );
